...
A Tax Module Provider is used to retrieve the tax lines in a provided context. The Tax Module provides a default \`system\` provider. You can create your own Tax Module Provider, either in a plugin, in a module provider, or directly in your vikrai application's codebase, then use it in any tax region.`,
      `## Understanding Tax Module Provider Implementation

The Tax Module Provider handles calculating taxes with a third-party provirder. However, it's not responsible for managing tax concepts within vikrai, such as creating a tax region. The Tax Module uses your Tax Module Provider within core operations.
...
If you're creating the module provider in a vikrai application, create it under the \`src/modules\` directory. For example, \`src/modules/my-tax\`.
...
Create the file \`src/modules/my-tax/service.ts\` that holds the module provider's main service. It must implement the \`ITaxProvider\` interface imported from \`@vikrai/framework/types\`:

\`\`\`ts title="src/modules/my-tax/service.ts"
import { ITaxProvider } from "@vikrai/framework/types"
...
To use your Tax Module Provider, add it to the \`providers\` array of the Tax Module in \`vikrai-config.ts\`:
