...
function formatEventsType(
  ...
  eventProperties.forEach((event, index) => {
    ...
    content.push("")
    content.push(
      `${subHeaderPrefix}# Workflows Emitting this Event\n\nThe following workflows emit this event when they're executed. These workflows are executed by vikrai's API routes. You can also view the events emitted by API routes in the [Store](https://docs.vikrai.com/api/store) and [Admin](https://docs.vikrai.com/api/admin) API references.`
    )
    content.push("")
    workflows?.forEach((workflow) => {
      content.push(`- [${workflow}](/references/vikrai-workflows/${workflow})`)
    })
    content.push("")
