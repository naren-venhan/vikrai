    ...
    describe("Notifications", () => {
      ...
      describe("Notifications module", () => {
        it("should successfully send a notification for an available channel", async () => {
          ...
          expect(logSpy).toHaveBeenCalledWith(
            `Attempting to send a notification to: 'test@vikrai.com' on the channel: 'email' with template: 'order-created' and data: '{\"username\":\"john-doe\"}'`
          )
        })
      ...
      describe("Configurable notification subscriber", () => {
        ...
        it("should successfully sent a notification when an order is created (based on configuration)", async () => {
          ...
          expect(logSpy).toHaveBeenCalledWith(
            `Attempting to send a notification to: 'test@vikrai.com' on the channel: 'email' with template: 'order-created-template' and data: '{\"order_id\":\"1234\"}'`
          )
          expect(notifications).toHaveLength(1)
          expect(notifications[0]).toEqual(
            expect.objectContaining({
