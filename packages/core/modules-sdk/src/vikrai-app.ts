import { RemoteFetchDataCallback } from "@vikrai/orchestration"
import {
...
export type vikraiModuleConfig = {
  [key: string | Modules]:
    | string
    | boolean
...
export async function loadModules(args: {
  modulesConfig: vikraiModuleConfig
  ...
  const loaded = (await vikraiModule.bootstrapAll(modulesToLoad, {
    migrationOnly,
    loaderOnly,
    workerMode,
  })) as LoadedModule[]
...
function getLoadedSchema(): string {
  return vikraiModule.getAllJoinerConfigs()
...
function registerCustomJoinerConfigs(servicesConfig: ModuleJoinerConfig[]) {
  for (const config of servicesConfig) {
    ...
    vikraiModule.setJoinerConfig(config.serviceName, config)
  }
}

export type vikraiAppOutput = {
...
async function vikraiApp_({
  ...
  migrationOnly?: boolean
} = {}): Promise<vikraiAppOutput> {
  const sharedContainer_ = createvikraiContainer({}, sharedContainer)

  const onApplicationShutdown = async () => {
    await promiseAll([
      vikraiModule.onApplicationShutdown(),
      sharedContainer_.dispose(),
    ])
  }

  const onApplicationPrepareShutdown = async () => {
    await promiseAll([vikraiModule.onApplicationPrepareShutdown()])
  }

  const onApplicationStart = async () => {
    await vikraiModule.onApplicationStart()
  }

  const modules: vikraiModuleConfig =
    modulesConfig ??
    (
      await dynamicImport(
        modulesConfigPath ??
  ...
  linkModules.push(...vikraiModule.getCustomLinks())

  const allLoadedJoinerConfigs = vikraiModule.getAllJoinerConfigs()
  ...
  const { remoteLink, getMigrationPlanner } = await initializeLinks({
    ...
    linkModules,
    injectedDependencies,
    moduleExports: isvikraiModule(linkModule) ? linkModule : undefined,
  })
  ...
  const applyMigration = async ({
    ...
    const moduleResolutions = modulesNames.map((moduleName) => {
      return {
        moduleName,
        resolution: vikraiModule.getModuleResolutions(moduleName),
      }
    })
    ...
    if (missingModules.length) {
      const error = new vikraiError(
        vikraiError.Types.UNKNOWN_MODULES,
        `Cannot ${action} migrations for unknown module(s) ${missingModules.join(
          ","
        )}`,
        vikraiError.Codes.UNKNOWN_MODULES
      )
    ...
    for (const { resolution: moduleResolution } of moduleResolutions) {
      ...
      if (action === "revert") {
        await vikraiModule.migrateDown(migrationOptions)
      } else if (action === "run") {
        await vikraiModule.migrateUp(migrationOptions)
      } else {
        await vikraiModule.migrateGenerate(migrationOptions)
      }
    }
  }
...
export async function vikraiApp(
  options: vikraiAppOptions = {}
): Promise<vikraiAppOutput> {
  return await vikraiApp_(options)
}

export async function vikraiAppMigrateUp(
  options: vikraiAppOptions = {}
): Promise<void> {
  const migrationOnly = true

  const { runMigrations } = await vikraiApp_({
    ...options,
    migrationOnly,
  })

  await runMigrations().finally(vikraiModule.clearInstances)
}

export async function vikraiAppMigrateDown(
  moduleNames: string[],
  options: vikraiAppOptions = {}
): Promise<void> {
  const migrationOnly = true

  const { revertMigrations } = await vikraiApp_({
    ...options,
    migrationOnly,
  })

  await revertMigrations(moduleNames).finally(vikraiModule.clearInstances)
}

export async function vikraiAppMigrateGenerate(
  moduleNames: string[],
  options: vikraiAppOptions = {}
): Promise<void> {
  const migrationOnly = true

  const { generateMigrations } = await vikraiApp_({
    ...options,
    migrationOnly,
  })

  await generateMigrations(moduleNames).finally(vikraiModule.clearInstances)
}

export async function vikraiAppGetLinksExecutionPlanner(
  options: vikraiAppOptions = {}
): Promise<ILinkMigrationsPlanner> {
  const migrationOnly = true

  const { linkMigrationExecutionPlanner } = await vikraiApp_({
    ...options,
    migrationOnly,
  })
