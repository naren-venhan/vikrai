...
const defaultContext = { __type: "vikraiContext" }
...
describe("Internal Module Service Factory", () => {
  ...
  const IvikraiInternalService = vikraiInternalService<any>(Model)

  describe("Internal Module Service Methods", () => {
    let instance

    beforeEach(() => {
      instance = new IvikraiInternalService(containerMock)
    })
    ...
    it("should throw an error on retrieve if composite key values are not defined", async () => {
      ...
      const compositeIvikraiInternalService =
        vikraiInternalService<any>(CompositeModel)

      const instance = new compositeIvikraiInternalService(containerMock)
