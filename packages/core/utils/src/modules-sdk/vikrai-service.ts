...
import { EmitEvents, InjectManager, vikraiContext } from "./decorators"
...
/**
 * Accessible from the vikraiService, holds the model objects when provided
 */
export const vikraiServiceModelObjectsSymbol = Symbol.for(
  "vikraiServiceModelObjectsSymbol"
)

/**
 * Symbol to mark a class as a vikrai service
 */
export const vikraiServiceSymbol = Symbol.for("vikraiServiceSymbol")

/**
 * Accessible from the vikraiService, holds the model name to linkable keys map
 * to be used for softDelete and restore methods
 */
export const vikraiServiceModelNameToLinkableKeysMapSymbol = Symbol.for(
  "vikraiServiceModelNameToLinkableKeysMapSymbol"
)

/**
 * Check if a value is a vikrai service
 * @param value
 */
export function isvikraiService(
  value: any
): value is vikraiServiceReturnType<any> {
  return value && value?.prototype[vikraiServiceSymbol]
}
 ...
 * }
 *
 * class MyService extends ModulesSdkUtils.vikraiService(models) {}
 *
 * @param models
 */
export function vikraiService<
...
> {
  ...
  const buildAndAssignMethodImpl = function (
    ...
    const applyMethod = function (impl: Function, contextIndex) {
      ...
      vikraiContext()(klassPrototype, methodName, contextIndex)
      ...
      case "softDelete":
        methodImplementation = async function <T extends { id: string }>(
          ...
          const mappedCascadedModelsMap = mapObjectTo(
            cascadedModelsMap,
            this[vikraiServiceModelNameToLinkableKeysMapSymbol],
            {
  ...
  class AbstractModuleService_ {
    [vikraiServiceSymbol] = true

    static [vikraiServiceModelObjectsSymbol] =
      models as unknown as vikraiServiceReturnType<
        ModelsConfig extends { __empty: any }
          ? ModelConfigurationsToConfigTemplate<TModels>
          : ModelsConfig
      >["$modelObjects"];

    [vikraiServiceModelNameToLinkableKeysMapSymbol]: MapToConfig
