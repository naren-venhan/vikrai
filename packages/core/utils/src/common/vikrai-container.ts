...
export function createvikraiContainer(...args): vikraiContainer {
  const container = createContainer.apply(null, args) as vikraiContainer

  container.registerAdd = registerAdd.bind(container)

  const originalScope = container.createScope
  container.createScope = () => {
    const scoped = originalScope() as vikraiContainer
