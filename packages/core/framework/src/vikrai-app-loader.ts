import {
  vikraiApp,
  vikraiAppGetLinksExecutionPlanner,
  vikraiAppMigrateDown,
  vikraiAppMigrateGenerate,
  vikraiAppMigrateUp,
  vikraiAppOutput,
  ModulesDefinition,
...
export class vikraiAppLoader {
  /**
   * Container from where to resolve resources
   * @private
   */
  readonly #container: vikraiContainer
  ...
  /**
   * Run, Revert or Generate the migrations for the vikrai app.
   *
   * @param moduleNames
   * @param linkModules
   * @param action
  ...
  async runModulesMigrations(
    ...
    if (action === "revert") {
      await vikraiAppMigrateDown(moduleNames!, migrationOptions)
    } else if (action === "run") {
      await vikraiAppMigrateUp(migrationOptions)
    } else {
      await vikraiAppMigrateGenerate(moduleNames!, migrationOptions)
    }
  }
  ...
  async getLinksExecutionPlanner(): Promise<ILinkMigrationsPlanner> {
    ...
    return await vikraiAppGetLinksExecutionPlanner(migrationOptions)
  }
   ...
   * @param config
   */
  async load(config = { registerInContainer: true }): Promise<vikraiAppOutput> {
    ...
    const vikraiApp = await vikraiApp({
    ...
    for (const moduleService of Object.values(vikraiApp.modules)) {
