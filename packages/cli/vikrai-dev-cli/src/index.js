...
cli.command({
  command: `*`,
  description: `Start the vikrai dev CLI`,
  builder: (yargs) => {
    yargs
      .usage(`Usage: vikrai-dev [options]`)
      .alias(`q`, `quiet`)
      .nargs(`q`, 0)
...
You typically only need to configure this once.`
      ...
      .describe(
        `C`,
        `Copy all contents in packages/ instead of just vikrai packages`
      )
      .array(`packages`)
      ...
      .nargs(`v`, 0)
      .alias(`v`, `version`)
      .describe(`v`, `Print the currently installed version of vikrai Dev CLI`)
  },
  handler: async (argv) => {
    ...
    if (pathToRepo) {
      ...
      conf.set(`vikrai-location`, path.resolve(pathToRepo))
      process.exit()
    }
...
`
    ...
    try {
      const monoRepoPkg = JSON.parse(
        fs.readFileSync(path.join(vikraiLocation, "package.json"))
      )
      for (const workspace of monoRepoPkg.workspaces.packages) {
        ...
        const workspacePackageDirs = glob.sync(workspace, {
          cwd: vikraiLocation.toString(),
        })
        monoRepoPackagesDirs.push(...workspacePackageDirs)
      }
    } catch (err) {
      console.error(
        `Unable to read and parse the workspace definition from vikrai package.json`
      )
      process.exit(1)
    }
    ...
    const monoRepoPackages = monoRepoPackagesDirs.map((dirName) => {
      try {
        const localPkg = JSON.parse(
          fs.readFileSync(path.join(vikraiLocation, dirName, `package.json`))
        )

        if (localPkg?.name) {
          packageNameToPath.set(
            localPkg.name,
            path.join(vikraiLocation, dirName)
          )
      ...
      packageNameToPath.set(dirName, path.join(vikraiLocation, dirName))
      return dirName
    })
...
`
    ...
    watch(vikraiLocation, argv.packages, {
      localPackages,
      quiet: argv.quiet,
