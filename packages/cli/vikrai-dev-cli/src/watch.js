...
async function watch(
  ...
  const realCopyPath = (arg) => {
    ...
    fs.copy(oldPath, newPath, (err) => {
      ...
      // When the vikrai binary is copied over, it is not setup with the executable
      // permissions that it is given when installed via yarn.
      // This fixes the issue where after running vikrai-dev, running `yarn vikrai develop`
      // fails with a permission issue.
      // @fixes https://github.com/vikrai/vikrai/issues/18809
      // Binary files we target:
      // - vikrai/bin/vikrai.js
      //  -vikrai/cli.js
      //  -vikrai-cli/cli.js
      if (/(bin\/vikrai.js|vikrai(-cli)?\/cli.js)$/.test(newPath)) {
  ...
  const allPackagesIgnoringThemesToWatch = allPackagesToWatch.filter(
    (pkgName) => !pkgName.startsWith(`vikrai-theme`)
  )
  ...
  chokidar
    ...
    .on(`all`, async (event, filePath) => {
      ...
      // Don't copy over the vikrai bin file as that breaks the NPM symlink.
      if (_.includes(filePath, `dist/vikrai-cli.js`)) {
        return
      }
      ...
      if (relativePackageFile === `package.json`) {
        ...
        if (didDepsChanged) {
          if (isInitialScan) {
            ...
            // vikrai package - like for example `vikrai-plugin-page-creator`
            // we need to publish both `vikrai-plugin-page-creator` and `vikrai`
            // and install `vikrai` in example site project.
