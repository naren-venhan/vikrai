...
import { setTelemetryEnabled } from "@vikrai/telemetry"
...
function buildLocalCommands(cli, isLocalProject) {
  ...
  function resolveLocalCommand(command) {
    ...
    try {
      const cmdPath = resolveCwd.silent(`@vikrai/vikrai/commands/${command}`)!
  ...
  cli
    .command({
      ...
      builder: (_) =>
        _.option(`seed`, {
          ...
          .option(`v2`, {
            type: `boolean`,
            describe: `Install vikrai with the V2 feature flag enabled. WARNING: vikrai V2 is still in development and shouldn't be used in production.`,
            default: false,
          })
          .option(`branch`, {
            type: `string`,
      ...
      desc: `Create a new vikrai project.`,
    ...
    .command({
      ...
      desc: "Sync database schema with the links defined by your application and vikrai core",
      builder: (builder) => {
    ...
    .command({
      ...
      builder: (_) =>
        _.option("types", {
          ...
          default: true,
          describe:
            "Generate automated types for modules inside the .vikrai directory",
        })
          .option(`H`, {
            alias: `host`,
            type: `string`,
    ...
    .command({
      ...
      builder: (_) =>
        _.option("admin-only", {
          ...
          type: "boolean",
          describe:
            "Only build the admin to serve it separately (outDir .vikrai/admin)",
        }),
      handler: handlerP(
...
function isLocalvikraiProject() {
  let invikraiProject = false
  ...
  try {
    ...
    invikraiProject = !!(
      (dependencies && dependencies["@vikrai/vikrai"]) ||
      (devDependencies && devDependencies["@vikrai/vikrai"])
    )
  } catch (err) {
    // ignore
  }

  return invikraiProject
}

function getVersionInfo() {
  const { version } = require(`../package.json`)
  const isvikraiProject = isLocalvikraiProject()
  if (isvikraiProject) {
    let vikraiVersion = ""
    try {
      vikraiVersion = require(path.join(
        process.cwd(),
        `node_modules`,
        `@vikrai/vikrai`,
        `package.json`
    ...
    if (!vikraiVersion) {
      vikraiVersion = `unknown`
    }

    return `vikrai CLI version: ${version}
vikrai version: ${vikraiVersion}
  Note: this is the vikrai version for the site at: ${process.cwd()}`
  } else {
    return `vikrai CLI version: ${version}`
  }
}
...
export default (argv) => {
  const cli = yargs()
  const isLocalProject = isLocalvikraiProject()
  ...
  try {
    cli.version(
      `version`,
      `Show the version of the vikrai CLI and the vikrai package in the current project`,
      getVersionInfo()
    )
  } catch (e) {
    // ignore
