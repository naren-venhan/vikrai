...
import { Pool } from "pg"
import url from "url"
import { track } from "@vikrai/telemetry"
// @ts-ignore
import inquirer from "inquirer"
import { createDatabase } from "pg-god"

import { getNodeVersion, MIN_SUPPORTED_NODE_VERSION } from "@vikrai/utils"
...
const createInitialGitCommit = async (rootPath, starterUrl) => {
  ...
  try {
    execSync(`git commit -m "Initial commit from vikrai: (${starterUrl})"`, {
      cwd: rootPath,
    })
  } catch {
...
const copy = async (starterPath, rootPath) => {
  ...
  if (starterPath === `.`) {
    throw new Error(
      ...
      something like "vikrai new my-vikrai-store ../local-vikrai-starter"`
    )
  }
  ...
  if (!starterPath && !rootPath) {
    const response = await prompts.prompt([
      ...
      !v2 && {
        ...
        choices: [
          { title: `vikrai-starter-default`, value: `vikrai-starter-default` },
    ...
    starterPath = `vikrai/${v2 ? "vikrai-starter-default" : response.starter}`
    rootPath = response.path
  }

  // set defaults if no root or starter has been set yet
  rootPath = rootPath || process.cwd()
  starterPath = starterPath || `vikrai/vikrai-starter-default`

  return { starterPath, rootPath, selectedOtherStarter }
}

const successMessage = (path) => {
  reporter.info(`Your new vikrai project is ready for you! To start developing run:
...
export const newStarter = async (args) => {
  ...
  if (nodeVersion < MIN_SUPPORTED_NODE_VERSION) {
    reporter.error(
      `vikrai requires at least v20 of Node.js. You're using v${nodeVersion}. Please install at least v20 and try again: https://nodejs.org/en/download`
    )
    process.exit(1)
  }
  track("CLI_NEW")
  ...
  if (selectedOtherStarter) {
    reporter.info(
      `Find the url of the vikrai starter you wish to create and run:

vikrai new ${rootPath} [url-to-starter]
...
`
  ...
  if (urlObject.protocol && urlObject.host) {
    ...
    if (/vikrai-starter/gi.test(rootPath) && isStarterAUrl) {
      reporter.panic({
  ...
  const dbName = `vikrai-db-${Math.random().toString(36).substring(2, 7)}` // generate random 5 character string
  ...
  if (creds === null) {
    reporter.info(
      "Skipping automatic database setup. Please note that you need to create a database and run migrations before you can run your vikrai backend"
    )
  } else {
    if (!skipDb) {
