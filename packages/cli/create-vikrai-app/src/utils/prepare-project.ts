...
async function prepareProject({
  ...
  let env = `vikrai_ADMIN_ONBOARDING_TYPE=${onboardingType}${EOL}STORE_CORS=${STORE_CORS}${EOL}ADMIN_CORS=${ADMIN_CORS}${EOL}AUTH_CORS=${AUTH_CORS}${EOL}REDIS_URL=${DEFAULT_REDIS_URL}${EOL}JWT_SECRET=supersecret${EOL}COOKIE_SECRET=supersecret`
  ...
  if (nextjsDirectory) {
    env += `${EOL}vikrai_ADMIN_ONBOARDING_NEXTJS_DIRECTORY=${nextjsDirectory}`
  }
  ...
  if (!skipDb && migrations) {
    ...
    await processManager.runProcess({
      process: async () => {
        const proc = await execute(["npx vikrai db:migrate", npxOptions], {
          verbose,
          needOutput: true,
        })
    ...
    await processManager.runProcess({
      process: async () => {
        const proc = await execute(
          [`npx vikrai user -e ${ADMIN_EMAIL} --invite`, npxOptions],
    ...
    if (apiKeys.rowCount) {
      ...
      fs.writeFileSync(
        ...
        originalContent.replace(
          "NEXT_PUBLIC_vikrai_PUBLISHABLE_KEY=pk_test",
          `NEXT_PUBLIC_vikrai_PUBLISHABLE_KEY=${apiKeys.rows[0].token}`
        )
      )
    }
  }
