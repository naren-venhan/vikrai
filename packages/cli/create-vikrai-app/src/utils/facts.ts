...
const facts = [
  ...
  "Tax-inclusive pricing allows you to set prices for products and shipping options while delegating tax calculations to vikrai.",
  "vikrai provides multi-currency and region support, with full control over prices for each currency and region.",
  ...
  "Data models represent tables in the database. They are created using vikrai's Data Modeling Language (DML).",
  "vikrai's store API routes are prefixed by /store. The admin API routes are prefixed by /admin.",
  "The JS SDK allows you to send requests to the vikrai server from your storefront or admin customizations.",
  ...
  "Infrastructure Modules are interchangeable modules that implement features and integrations related to the vikrai server's infrastructure.",
...
export const showFact = ({
  ...
  if (verbose) {
    ...
    spinner.text = `${title}\n${boxen(`${fact}`, {
      title: chalk.cyan(`${emojify(":bulb:")} vikrai Tips`),
