...
import { PluginProjectCreator } from "./vikrai-plugin-creator.js"
import { vikraiProjectCreator } from "./vikrai-project-creator.js"
...
export class ProjectCreatorFactory {
  static async create(
    ...
    return options.plugin
      ...
      : new vikraiProjectCreator(projectName, options, args)
  }
  ...
  private static validateNodeVersion(): void {
    ...
    if (nodeVersion < MIN_SUPPORTED_NODE_VERSION) {
      logMessage({
        message: `vikrai requires at least v20 of Node.js. You're using v${nodeVersion}. Please install at least v20 and try again: https://nodejs.org/en/download`,
        type: "error",
      })
    }
  }
  ...
  const { projectName } = await inquirer.prompt([
    {
      ...
      default: isPlugin ? "my-vikrai-plugin" : "my-vikrai-store",
      filter: (input) => {
