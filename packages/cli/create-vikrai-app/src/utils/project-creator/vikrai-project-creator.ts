...
import startvikrai from "../start-vikrai.js"
import {
  BaseProjectCreator,
  ProjectCreator,
  ProjectOptions,
...
// vikrai Project Creator
export class vikraiProjectCreator
  ...
  implements ProjectCreator
{
  ...
  private async setupDatabase(): Promise<void> {
    let dbName = `vikrai-${slugify(this.projectName)}`
    ...
    logMessage({
      message: boxen(
        chalk.green(
          `Change to the \`${
            this.projectName
          }\` directory to explore your vikrai project.${EOL}${EOL}Start your vikrai application again with the following command:${EOL}${EOL}yarn dev${EOL}${EOL}${
            this.inviteToken
              ? `After you start the vikrai application, you can create an admin user with the URL http://localhost:9000/app/invite?token=${this.inviteToken}&first_run=true${EOL}${EOL}`
              : ""
          }${
            this.nextjsDirectory?.length
              ...
              : ""
          }Check out the vikrai documentation to start your development:${EOL}${EOL}https://docs.vikrai.com/${EOL}${EOL}Star us on GitHub if you like what we're building:${EOL}${EOL}https://github.com/vikrai/vikrai/stargazers`
        ),
        {
