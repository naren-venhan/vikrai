...
This command will scan the `@vikrai/vikrai` package in order to extract JSDoc OAS into a json file.
...
```bash
yarn vikrai-oas oas --type admin
```
...
```bash
yarn vikrai-oas oas --paths ~/vikrai-server/src
```
...
```bash
yarn vikrai-oas oas --base ~/vikrai-server/oas/custom.oas.base.yaml
```
...
```bash
yarm vikrai-oas docs --src-file ./store.oas.json
```
...
```bash
yarn vikrai-oas docs --src-file ./store.oas.json --out-dir ./docs`
```
...
```bash
yarn vikrai-oas --src-file ./store.oas.json --config ./redocly-config.yaml
```
...
```bash
yarn vikrai-oas docs --src-file ./store.oas.json --dry-run
```
...
```bash
yarn vikrai-oas docs --src-file ./store.oas.json --clean
```
...
```bash
yarn vikrai-oas docs --src-file ./store.oas.json --split
```
...
```bash
yarn vikrai-oas docs --src-file ../../../www/apps/api-reference/specs/store.oas.json --preview
```
...
```bash
yarn vikrai-oas docs --src-file ./store.oas.json --html
```
