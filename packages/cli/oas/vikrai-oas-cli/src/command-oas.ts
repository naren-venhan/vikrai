 ...
 * Constants
 */
// vikrai core package directory
const vikraiPackagePath = path.dirname(require.resolve("@vikrai/vikrai"))
...
export const commandOptions: Option[] = [
  ...
  new Option(
    "--local",
    "Generate OAS from local files rather than public OAS. This is useful for generating references in the vikrai monorepo."
  ),
]
...
async function getOASFromCodebase(apiType: ApiType): Promise<OpenAPIObject> {
  ...
  const gen = await swaggerInline(
    [
      ...
      path.resolve(vikraiPackagePath, "dist", "utils/middlewares"),
    ],
    {
...
async function getPublicOas(apiType: ApiType) {
  const url = `https://docs.vikrai.com/api/download/${apiType}`
