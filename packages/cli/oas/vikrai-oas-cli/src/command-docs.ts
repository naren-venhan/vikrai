...
const vikraiPluginRelativePath = "./plugins/vikrai/index.js"
const vikraiPluginAbsolutePath = path.resolve(
  basePath,
  "redocly/plugins/vikrai/index.js"
)
...
const createTmpConfig = async (
  ...
  config.plugins = (config.plugins ?? []).filter(
    (plugin) => plugin !== vikraiPluginRelativePath
  )
  config.plugins.push(vikraiPluginAbsolutePath)
...
`
      ...
      Object.keys(recommendation).forEach((recKey) => {
        originalContent.decorators["vikrai/circular-patch"].schemas[recKey] = [
          ...(originalContent.decorators["vikrai/circular-patch"].schemas[
            recKey
          ] || []),
