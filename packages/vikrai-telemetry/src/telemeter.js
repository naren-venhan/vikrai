...
class Telemeter {
  ...
  getvikraiVersion() {
    try {
      const packageJson = require.resolve(`@vikrai/vikrai/package.json`)
      const { version } = JSON.parse(fs.readFileSync(packageJson, `utf-8`))
      return version
    } catch (e) {
      if (isTruthy(vikrai_TELEMETRY_VERBOSE)) {
        console.error("failed to get vikrai version", e)
      }
    }
    return `-0.0.0`
  }
  ...
  getCliVersion() {
    try {
      const jsonfile = join(
        require.resolve(`@vikrai/cli`).split(`${sep}dist`).shift(),
        "package.json"
      )
  ...
  enqueue_(type, data) {
    const event = {
      ...
      vikrai_version: this.getvikraiVersion(),
