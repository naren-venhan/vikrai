import { getConfigFile } from "@vikrai/framework/utils"

export async function configLoaderOverride(
  entryDirectory: string,
  override: { clientUrl: string; debug?: boolean }
) {
  const { configManager } = await import("@vikrai/framework/config")
  const { logger } = await import("@vikrai/framework")
  const { configModule, error } = await getConfigFile<
    ReturnType<typeof configManager.loadConfig>
  >(entryDirectory, "vikrai-config")
