...
import qs from "qs"
import { ApiLoader } from "@vikrai/framework/http"
import { vikraiContainer, PluginDetails } from "@vikrai/framework/types"
import { ConfigModule } from "@vikrai/framework/config"
...
export default async ({ app, container, plugins }: Options) => {
  ...
  /**
   * Always load plugin routes before the vikrai core routes, since it
   * will allow the plugin to define routes with higher priority
   * than vikrai. Here are couple of examples.
   *
   * - Plugin registers a route called "/products/active"
   * - vikrai registers a route called "/products/:id"
   *
   * Now, if vikrai routes gets registered first, then the "/products/active"
  ...
  // Test: (packages/vikrai/src/api/routes/admin/currencies/update-currency.ts)
  try {
    await new ApiLoader({
      app: app,
