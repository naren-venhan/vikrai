...
export default async function adminLoader({
  ...
  if (NOT_ALLOWED_PATHS.includes(adminOptions.path)) {
    logger.error(
      `The 'admin.path' in 'vikrai-config.js' is set to a value that is not allowed. This can prevent your server from working correctly. Please set 'admin.path' to a value that is not one of the following: ${NOT_ALLOWED_PATHS.join(
        ", "
      )}.`
    )
  }
...
async function initDevelopmentServer(app: Express, options: IntializedOptions) {
  const { develop } = await import("@vikrai/admin-bundler")
...
async function serveProductionBuild(app: Express, options: IntializedOptions) {
  const { serve } = await import("@vikrai/admin-bundler")
