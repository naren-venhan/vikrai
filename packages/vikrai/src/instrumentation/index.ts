...
import { Tracer } from "@vikrai/framework/telemetry"
import type { SpanExporter } from "@opentelemetry/sdk-trace-node"
import type { NodeSDKConfiguration } from "@opentelemetry/sdk-node"
import { TransactionOrchestrator } from "@vikrai/framework/orchestration"
...
export function instrumentHttpLayer() {
  ...
  const HTTPTracer = new Tracer("@vikrai/http", "2.0.0")
...
export function instrumentRemoteQuery() {
  const QueryTracer = new Tracer("@vikrai/query", "2.0.0")
  const { SpanStatusCode } = require("@opentelemetry/api")
...
export function instrumentWorkflows() {
  const WorkflowsTracer = new Tracer(
    "@vikrai/framework/workflows-sdk",
    "2.0.0"
  )
