...
import { MigrationScriptsMigrator } from "@vikrai/framework/migrations"
import {
...
import { vikraiContainer, PluginDetails } from "@vikrai/types"
...
export async function runMigrationScripts({
  ...
  let container_: vikraiContainer
  ...
  try {
    ...
    const scriptsSourcePaths = [
      join(dirname(require.resolve("@vikrai/vikrai")), "migration-scripts"),
...
async function loadResources(plugins: any): Promise<{
  ...
  const vikraiAppResources = await new vikraiAppLoader().load()
  const onApplicationPrepareShutdown =
    vikraiAppResources.onApplicationPrepareShutdown
  const onApplicationShutdown = vikraiAppResources.onApplicationShutdown
  await vikraiAppResources.onApplicationStart()
