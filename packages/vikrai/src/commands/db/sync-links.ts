...
import { LinkMigrationsPlannerAction } from "@vikrai/framework/types"
import {
...
export async function syncLinks(
  vikraiAppLoader: vikraiAppLoader,
  {
    executeAll,
    executeSafe,
  }: {
  ...
  const planner = await vikraiAppLoader.getLinksExecutionPlanner()
...
const main = async function ({ directory, executeSafe, executeAll }) {
  try {
    ...
    await syncLinks(vikraiAppLoader, { executeAll, executeSafe })
    process.exit()
  } catch (error) {
    logger.error(error)
    process.exit(1)
